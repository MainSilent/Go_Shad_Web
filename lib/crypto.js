...
function encrypt(auth, data) { 
    return new Promise((resolve, reject) => {
        exec(`java "${__dirname}\\Crypto\\Crypto.java" ${auth} -e ${JSON.stringify(data).replace(/"/g, "\\\"").replace(/ /g, '')}`, (error, stdout, stderr) => {
            if (error) {
...
function decrypt(auth, data_enc) { 
    return new Promise((resolve, reject) => {
        const tmp_path = __dirname + "\\tmp\\" + Str.random(128)
        fs.writeFile(tmp_path, data_enc.replace(/\n/g, ''), function(err) {
            if (err) {
               return console.log(err)
            }
            exec(`java "${__dirname}\\Crypto\\Crypto.java" ${auth} -d "${tmp_path}"`, (error, stdout, stderr) => {
                if (error) {
